...
PUBLIC void init_prot()
{
	...
	init_idt_desc(0x80, DA_386IGate, sys_call_ex, PRIVILEGE_USER);

	/* 填充 GDT 中 tss 这个描述符 */
	memset(&tss, 0, sizeof(tss));
	tss.ss0 = SELECTOR_KERNEL_DS;
	init_desc(&gdt[INDEX_TSS], (u32) &tss, sizeof(tss) - 1,
	DA_386TSS);
	...
	for (i = 0; i < NR_TASKS; i++)
	{
		...
		init_desc(&gdt[INDEX_LDT_FIRST + i], (u32) &proc_table_task[i].ldts,
